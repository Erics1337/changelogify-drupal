...
shows profile and length controls, with Auto grouping selected by default,
optional category/source/evidence exclusions, and the exact post-eligibility,
post-exclusion, privacy-filtered JSON. Changing an exclusion requires updating
that preview before the provider request begins. The deterministic commit
button and its section assignments remain unchanged.
